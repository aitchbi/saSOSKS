function y = spgg_kernel_meyer_highpass(x,t,w,vers)
% SPGG_kernel_highpass highpass kernel using Meyer auxilary function, for a
% given transition point and width w. A complementary lowpass filter can
% be defiend (spgg_kernel_lowpass.m) using the same parameters as used
% here in the lowpass filter. The resulting spectral kernel pair will have
...
%
% Based on: spgg_kernel_2.m
%
% Also see:
% spgg_kernel_lowpass.m
%
%
% Hamid Behjat
% Nov 2020.
...
d = x>a & x<=M*a;
y(d) = sin(pi/2*(meyeraux(q*(x(d)/a -1))));
